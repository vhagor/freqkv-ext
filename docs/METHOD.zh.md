...
（$d_\mathrm{head} = 128$，$\mathrm{base} = 10000$，取 $N = 4096$）：

- $\theta_0 = 1$ rad/sample，$n_0 \approx 652$。
- $\theta_{16} = 0.1$ rad/sample，$n_{16} \approx 65$。
- $\theta_{32} = 0.01$ rad/sample，$n_{32} \approx 7$。
- $\theta_{63} \approx 10^{-4}$ rad/sample，$n_{63} \approx 0$。

H100 上的频谱分析实证刚好印证这一点：pair 0 的 post-RoPE 峰约在 bin 652，
pair 32 在 bin 7 附近，pair 63 在 bin 0。

FreqKV 的统一低通 $[0, L)$ 漏掉 $n_0$ 的条件是 $L < n_0 = 652$，即
$\gamma < n_0 / N \approx 0.16$。**在 FreqKV 默认 $\gamma = 0.5$ 下，
$[0, L)$ 远比所有 $n_i$ 都大，FreqKV 并没有丢掉任何 pair 的能量中心**——
RoPE-matched bandpass 相对它的 slack 只在"每个 channel 用同一窗 vs 每对 pair
用各自窗"的形状自适应层面，量级有限。**DFT-RoPE 的结构性优势在 $\gamma
\lesssim 0.16$ 才显现**，那里 FreqKV 开始把高 $\theta$ pair 的能量中心整段
切掉——对应 FreqKV 自己论文 Table 3 中 $\gamma \to 0.01$ 时 PPL 爆炸的区间。
